...
int pwc_decompress(struct pwc_device *pdev)
{
	...
	if (pdev->vpalette == VIDEO_PALETTE_RAW)
	{
		...
		raw_frame->type = pdev->type;//cpu_to_le16(pdev->type);
		raw_frame->vbandlength = pdev->vbandlength;//cpu_to_le16(pdev->vbandlength);
	...
	if (pdev->vbandlength == 0) {
		 ...
		 */
		src = (unsigned short *)yuv;
		n = pdev->view.x * pdev->view.y;

		/* offset in Y plane */
		stride = pdev->view.x * pdev->offset.y + pdev->offset.x;
		dsty = (unsigned short *)(image + stride);

		/* offsets in U/V planes */
		stride = pdev->view.x * pdev->offset.y / 4 + pdev->offset.x / 2;
		dstu = (unsigned short *)(image + n +         stride);
		dstv = (unsigned short *)(image + n + n / 4 + stride);
...
#if VERBOSE
        printf("Mode Bayer is not supported for now\n");
#endif
		/* flags |= PWCX_FLAG_BAYER; */
		return -3; /* No such device or address: missing decompressor */
	}
	...
	if (DEVICE_USE_CODEC1(pdev->type)) {
		...
		/* TODO & FIXME */
#if VERBOSE
        printf("This chipset is not supported for now\n");
#endif
		return -3; /* No such device or address: missing decompressor */
