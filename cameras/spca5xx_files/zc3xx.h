...
static __u8 zcxx_i2cWrite(struct usb_device *dev,__u8 reg ,__u8 valL,__u8 valH)
{
__u8 retbyte = 0;
	spca5xxRegWrite(dev,0xa0,reg,0x92,NULL,0);
...
/* check SIF */
	...
	retbyte = zcxx_i2cWrite(spca50x->dev,zcxxi2cSensorSIF[i][SensorReg1],
			zcxxi2cSensorSIF[i][valSreg1L],zcxxi2cSensorSIF[i][valSreg1H]);
	  wait_ms(2);
	  retbyte = ( zcxx_i2cRead (spca50x->dev,zcxxi2cSensorSIF[i][SensorReg1])) & 0xff;
	 wait_ms(2);
	 ...
	 retbyte = zcxx_i2cWrite(spca50x->dev,zcxxi2cSensorSIF[i][SensorReg2],
	 	zcxxi2cSensorSIF[i][valSreg2L],zcxxi2cSensorSIF[i][valSreg2H]);
	  retbyte = ( zcxx_i2cRead (spca50x->dev,zcxxi2cSensorSIF[i][SensorReg2])) & 0xff; 
...
/* check VGA */
	...
	retbyte = zcxx_i2cWrite(spca50x->dev,zcxxi2cSensor[i][SensorReg1],
			zcxxi2cSensor[i][valSreg1L],zcxxi2cSensor[i][valSreg1H]);
	 if(zcxxi2cSensor[i][SensorReg2] != 0xff){
	retbyte = zcxx_i2cWrite(spca50x->dev,zcxxi2cSensor[i][SensorReg2],
			zcxxi2cSensor[i][valSreg2L],zcxxi2cSensor[i][valSreg2H]); 
	 retbyte = ( zcxx_i2cRead (spca50x->dev,zcxxi2cSensor[i][SensorReg2])) & 0xff; 
	 } else {
	 retbyte = ( zcxx_i2cRead (spca50x->dev,zcxxi2cSensor[i][SensorReg1])) & 0xff; 
	 }
...
/* check 3 wires bus */
	...
	while (zcxx3wrSensor [i][0]) {
	 ...
	 for (j =2 ; j < 5; j++){
		...
		retbyte = ( zcxx_i2cRead (spca50x->dev,zcxx3wrSensor[i][j])) & 0xff; 
			if (retbyte != 0)
				return (i | 0x10);
		}
	 }
	if (zcxx3wrSensor[i][1] == 0x91) {
		...
		checkword |= retbyte;
		PDEBUG(0,"sensor 3w Vga ??? 0x%04X ",(checkword & 0xffff) );
...
static void zc3xx_setbrightness(struct usb_spca50x *spca50x)
{
	...
	if(brightness < 0x70){
	spca5xxRegWrite(spca50x->dev,0xa0,brightness+0x10,0x018d,NULL,0);
	} else {
...
static void zc3xx_start(struct usb_spca50x *spca50x )
{
	...
	switch (spca50x->sensor){
		...
		case SENSOR_PB0330:
			if (spca50x->mode){
			/* 320x240 */
			if ( (spca50x->chip_revision == 0xc001) || (spca50x->chip_revision == 0xe001))
