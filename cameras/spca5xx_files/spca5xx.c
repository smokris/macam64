...
MODULE_PARM_DESC (RegStrobe, "Strobe to read or write a register 1=write, 2=read");
#endif /* SPCA5XX_ENABLE_REGISTERPLAY */
		  ...
		  int index, __u16 ext_modes[][6])
{
  ...
  else if (spca50x->bridge == BRIDGE_SN9CXXX )
    {
      mode = (ext_modes[index][2]) & 0x0F;
      method = (ext_modes[index][2] & 0xF0) >> 4;
      spca50x->mode = mode;
  ...
  else if (spca50x->bridge == BRIDGE_SPCA504C)
    {
...
spca50x_next_frame (struct usb_spca50x *spca50x, unsigned char *cdata)
{
  ...
  struct spca50x_frame *frame = NULL;
   PDEBUG(2, "Frame %d State %d",spca50x->curframe,spca50x->frame[spca50x->curframe].grabstate);
...
spca50x_move_data (struct usb_spca50x *spca50x, struct urb *urb)
{
  ...
  for (i = 0; i < urb->number_of_packets; i++)
	...
	spin_lock(&spca50x->v4l_lock);	
		ms_times_before = spca50x->last_times + spca50x->dtimes;
	spin_unlock(&spca50x->v4l_lock);
		if( ms_times_now >= ms_times_before){
 			PDEBUG(2,"Decode frame last %d",(int)(ms_times_now-spca50x->last_times));
...
spca50x_init_source (struct usb_spca50x *spca50x)
{
  ...
  if(spca50x->bridge == BRIDGE_ZC3XX ||
   ...
   if ((index = spca50x_find_mode_index (spca50x->width,
					spca50x->height,
					GET_EXT_MODES (spca50x->bridge))) == -EINVAL)
    {
...
static inline void spca5xx_chgAuto(struct usb_spca50x *spca50x, __u8 autoval)
{
...
static inline void spca5xx_chgQtable(struct usb_spca50x *spca50x, __u8 qtable)
{ 
...
static inline void spca5xx_chgDtimes(struct usb_spca50x *spca50x, __u16 dtimes)
{
	unsigned long flags;
...
spcaCameraShutDown (struct usb_spca50x *spca50x)
{
  if (spca50x->dev)
      ...
      else if (spca50x->bridge == BRIDGE_SPCA561)
	{
      ...
      else if (spca50x->bridge == BRIDGE_PAC207)
	{
...
if (RegStrobe != 0) {
	if (RegStrobe == 1) {
		...
		Rval = RegValue & 0xFF;
		spca5xxRegWrite(spca50x->dev,0xa0,Rval,(__u16)(RegAddress & 0xFFFF),NULL,0);
		}
	} else {
		if (spca50x->bridge == BRIDGE_PAC207)  {
...
product = dev->descriptor.idProduct;
  ...
  switch ( vendor )
	...
	case 0x430:
	  if (usbgrabber)
	  ...
	  else
	    {
	      ...
	      spca50x->cameratype = YYUV;
	      info
		("USB SPCA5XX camera found. Type Intel PC Camera Pro (SPCA505)");
	    }
	  break;
	...
	case 0x401:
	  ...
	  info
	    ("USB SPCA5XX camera found. Type Intel Create and Share (SPCA501 )");
	  break;
	case 0x402:
	...
	case 0x110:
	  ...
	  info
	    ("USB SPCA5XX camera found. Type ViewQuest (SPCA508?)");
	  break;
	 case 0x3261:
	...
	case 0x0002:
	  ...
	  spca50x->cameratype = YUYV;
	  info
	    ("USB SPCA5XX camera found. Type Kodak DVC-325 (SPCA501A )");
	  break;
	case 0x0300:
